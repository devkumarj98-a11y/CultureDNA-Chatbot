...
<style>
    ...
    .hero-wrap {
        background: linear-gradient(135deg, #fff8ef 0%, #fffdf8 52%, #f5fbff 100%);
        border: 1px solid #f1d7b5;
        border-radius: 26px;
        padding: 34px 34px 28px 34px;
        margin: 8px 0 22px 0;
        box-shadow: 0 12px 35px rgba(70,45,25,.08);
    }
    .hero-kicker {
        display:inline-block; padding:6px 12px; border-radius:999px;
        background:#fff0df; color:#9b4b16; font-weight:700; font-size:.82rem;
        letter-spacing:.3px; margin-bottom:12px;
    }
    .hero-title {
        font-family:'Playfair Display',serif; font-size:3rem; line-height:1.08;
        color:#2b211b; margin:0 0 10px 0;
    }
    .hero-text { color:#625a54; font-size:1.08rem; line-height:1.65; max-width:820px; margin-bottom:18px; }
    .hero-chip {
        display:inline-block; margin:5px 6px 0 0; padding:8px 13px; border-radius:999px;
        background:#ffffff; border:1px solid #ead8c5; color:#493d35; font-size:.88rem; font-weight:600;
    }
    .feature-card {
        background:#fff; border:1px solid #eee1d5; border-radius:18px; padding:18px;
        min-height:125px; box-shadow:0 6px 18px rgba(0,0,0,.04);
    }
    ...
    .section-label { color:#8a4a20; font-size:.8rem; font-weight:800; text-transform:uppercase; letter-spacing:1.2px; margin:18px 0 7px; }
...
if not st.session_state.get("sidebar_state_select") or st.session_state.get("sidebar_state_select") == "Select a state/region":
    st.markdown("""
    <div class="hero-wrap">
      <div class="hero-kicker">🇮🇳 SMART INDIA HACKATHON • CULTURAL DISCOVERY</div>
      <div class="hero-title">Discover India's Cultural DNA</div>
      ...
      <span class="hero-chip">🍛 Food</span><span class="hero-chip">👗 Dress</span><span class="hero-chip">💃 Dance</span><span class="hero-chip">🎵 Music</span><span class="hero-chip">🎉 Festivals</span><span class="hero-chip">🧵 Crafts</span><span class="hero-chip">🏛️ Heritage</span>
    </div>
    """, unsafe_allow_html=True)
    st.markdown('<div class="section-label">What you can explore</div>', unsafe_allow_html=True)
    f1,f2,f3,f4 = st.columns(4)
    cards = [
        ("🍛", "Cuisine", "Discover the exact traditional food associated with each region."),
        ("👗", "Traditions", "Explore dress, rituals, festivals and everyday heritage."),
        ("💃", "Performing Arts", "Find folk dances, music and other cultural expressions."),
        ("🏛️", "Heritage", "Explore monuments, architecture and cultural landmarks on map."),
    ]
    for col,(icon,title,desc) in zip([f1,f2,f3,f4],cards):
        with col:
            st.markdown(f'<div class="feature-card"><div class="feature-icon">{icon}</div><div class="feature-title">{title}</div><div class="feature-text">{desc}</div></div>', unsafe_allow_html=True)
