...
<html><head><meta charset="utf-8"><style>body{margin:0;font-family:Arial,sans-serif;background:transparent;color:#172b3a}button{width:100%;border:0;border-radius:14px;padding:12px 16px;font-size:16px;font-weight:700;background:#e96922;color:white;cursor:pointer}button.listening{background:#b52b24}#status{text-align:center;font-size:12px;margin-top:7px;color:#53616b;min-height:16px}</style></head>
<body><button id="mic">🎤 Start speaking</button><div id="status">Tap once — I'll stop automatically when you finish.</div>
<script>
const Streamlit=window.parent.Streamlit;const btn=document.getElementById('mic'),status=document.getElementById('status');const SR=window.SpeechRecognition||window.webkitSpeechRecognition;let recognition=null,listening=false,lang='en-IN';
...
function send(value){window.parent.postMessage({isStreamlitMessage:true,type:'streamlit:setComponentValue',value:value},'*');}
if(!SR){btn.disabled=true;status.textContent='Voice recognition is not supported in this browser.';}else{recognition=new SR();recognition.continuous=false;recognition.interimResults=false;recognition.maxAlternatives=1;recognition.onstart=()=>{listening=true;btn.classList.add('listening');btn.textContent='🔴 Listening…';status.textContent='Speak your question…';};recognition.onresult=(e)=>{const text=Array.from(e.results).map(r=>r[0].transcript).join(' ').trim();if(text){status.textContent='✅ Got it — searching…';send(text);}};recognition.onerror=(e)=>{status.textContent=e.error==='not-allowed'?'Microphone permission denied.':'Could not understand. Try again.';};recognition.onend=()=>{listening=false;btn.classList.remove('listening');btn.textContent='🎤 Start speaking';};btn.onclick=()=>{if(!listening){recognition.lang=lang;try{recognition.start();}catch(e){}}else recognition.stop();};}
try{Streamlit.setComponentReady();Streamlit.setFrameHeight(92);}catch(e){}
</script></body></html>'''
try:
