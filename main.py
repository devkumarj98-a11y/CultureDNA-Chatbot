...
def get_visual_for_reply(state: str, field: str = None, row=None, location: str = None, location_coords=None):
    """Return image, map coordinates and a relevant cultural-video search link."""
    ...
    if row is not None and "image_url" in row:
        val = str(row["image_url"]).strip()
        if val.startswith(("http://", "https://")):
            custom_image_url = val

    if not custom_image_url and row is not None and "Multimedia / Images" in row:
        val = str(row["Multimedia / Images"]).strip()
        match = re.search(r"https?://[^\s,;]+", val)
        if match:
            custom_image_url = match.group(0)

    if not custom_image_url and state in STATE_CUSTOM_IMAGE_URLS:
    ...
    if not custom_image_url and location:
        visual_result = fetch_cultural_visual(f"{location} {clean_name} India")
        if not visual_result:
            visual_result = fetch_cultural_visual(location)

    if not custom_image_url and not visual_result and field and row is not None:
        ...
        if cell_val:
            ...
            if len(first_item) > 2:
                visual_result = fetch_cultural_visual(f"{first_item} {location_label}")
                if not visual_result:
                    visual_result = fetch_cultural_visual(first_item)

    if not custom_image_url and not visual_result and state in STATE_COORDINATES:
        visual_result = fetch_cultural_visual(
            state_meta.get("query", f"{clean_name} landmark")
        )
    ...
    if field and row is not None:
        video_query = f"{location_label} {clean_name} {FIELD_LABELS.get(field, field)}"
...
def chatbot_reply(message: str, selected_state: str = None):
    ...
    if state:
        ...
        visual_data = get_visual_for_reply(
            state, field, row, location=location, location_coords=location_coords
        )
