...
ALIASES = {
    ...
    "mp": "Madhya Pradesh",
    "hp": "Himachal Pradesh",
    "uk": "Uttarakhand", "utrakhand": "Uttarakhand", "uttar khand": "Uttarakhand", "uttrakhand": "Uttarakhand",
    "ap": "Andhra Pradesh",
    "wb": "West Bengal",
    "bengal": "West Bengal",
    "orissa": "Odisha",
...
def geocode_location(place_query: str):
    ...
    try:
        ...
        return {
            ...
            "district_name": address.get("state_district") or address.get("district") or address.get("county", ""),
...
def find_state(text):
    text_low = re.sub(r"[^a-z0-9&]+", " ", str(text).lower()).strip()
    ...
    if images:
        ...
        for i, img in enumerate(images):
            with cols[i % len(cols)]:
                # Keep the item label outside the image so it is always visible on mobile.
                item_name = str(img.get("topic_item") or "Cultural subject").strip()
                st.image(img["image_url"], use_container_width=True)
                st.markdown(
                    f'<div class="cultural-item-name"><strong>{item_name}</strong></div>',
                    unsafe_allow_html=True,
                )
                # Plain Markdown fallback keeps the item name visible even if a
                # mobile browser applies unexpected CSS to the HTML label.
...
def chatbot_reply(message: str, selected_state: str = None):
    ...
    # Resolve arbitrary Indian districts/cities even when the state is already
    # present in the query, e.g. "Araria Bihar food". Known aliases are used
    # first; Nominatim is only called when a specific place still needs to be
    # identified.
    geocoded = None
    query_for_geo = re.sub(
        r"\b(what|which|tell|me|about|the|culture|of|in|from|is|are|"
        r"famous|popular|traditional|food|foods|cuisine|dish|dishes|dance|dances|"
        r"festival|festivals|heritage|monument|monuments|dress|clothing|attire|"
        r"music|craft|crafts|textile|textiles|jewellery|jewelry|history|ritual|"
        r"rituals|tradition|traditions|community|communities|tribe|tribes|"
        r"architecture|landmark|landmarks|occupation|occupations|theatre|theater|"
        r"performing|arts|knowledge|practices|district|city|town|village|area|"
        r"place|show|where|map|please|ke|ka|ki|mein|me|se|par|hai|kya|batao|"
        r"bataiye|famous)\b",
        " ", low, flags=re.IGNORECASE
    )
    for known_state in list_states():
        query_for_geo = re.sub(rf"\b{re.escape(str(known_state).lower())}\b", " ", query_for_geo, flags=re.IGNORECASE)
    query_for_geo = re.sub(r"\s+", " ", query_for_geo).strip()

    needs_geo = bool(query_for_geo and len(query_for_geo) >= 3 and not location)
    if needs_geo:
        geocoded = geocode_location(query_for_geo)
        if geocoded:
            geo_state = state_from_geocoded_name(geocoded.get("state_name", ""))
            if geo_state:
    ...
    if state:
        ...
        if is_specific_location:
            ...
            # Do not reuse state-level cultural images for a district/city question.
            # For a specific place, visuals must be tied to that place; otherwise the
            # app shows the location reference image (when available) or no image.
            visual_data = {
                "image_url": district_ref.get("image_url") if district_ref else None,
                "image_caption": district_ref.get("title", location) if district_ref else location,
                "images": [],
                "topic_items": [],
                "coords": location_coords or (district_ref.get("coords") if district_ref else None),
                "location_name": district_ref.get("title", location) if district_ref else location,
                "video_url": youtube_search_url(f"{location} {get_clean_region_name(state)} {FIELD_LABELS.get(field, field) if field else 'culture'} India"),
                "video_query": f"{location} {get_clean_region_name(state)} {FIELD_LABELS.get(field, field) if field else 'culture'} India",
                "videos": [{
                    "item": f"{location} {FIELD_LABELS.get(field, field) if field else 'Culture'}",
                    "url": youtube_search_url(f"{location} {get_clean_region_name(state)} {FIELD_LABELS.get(field, field) if field else 'culture'} India")
                }],
                "topic_item": None,
            }
            if field:
                district_text += f"\n\n**Requested category:** {FIELD_LABELS.get(field, field)}"
                if not district_ref or not district_ref.get("image_url"):
                    district_text += "\n\n*No district-specific image was substituted from the state dataset.*"
        ...
        if field:
            ...
            missing_values = {"", "nan", "none", "not documented in current dataset"}

            if value.lower() in missing_values:
                # The current district workbook is an administrative foundation and
                # some state-level cultural cells are intentionally empty. In that
                # case, use the existing reference lookup rather than stopping after
                # category detection. This also keeps misspelled state queries useful.
                fallback_text, fallback_ref = district_culture_reply(
                    display_name, state, field, text
                )
                if fallback_ref:
                    visual_data = {
                        "image_url": fallback_ref.get("image_url"),
                        "image_caption": fallback_ref.get("title", display_name),
                        "images": [],
                        "topic_items": [],
                        "coords": location_coords or fallback_ref.get("coords"),
                        "location_name": display_name,
                        "video_url": youtube_search_url(
                            f"{display_name} {FIELD_LABELS.get(field, field)} India"
                        ),
                        "video_query": f"{display_name} {FIELD_LABELS.get(field, field)} India",
                        "videos": [{
                            "item": f"{display_name} {FIELD_LABELS.get(field, field)}",
                ...
                reply_text = (
                    fallback_text
                    + "\n\n*Reference lookup used because the current district "
                      "workbook does not yet contain a populated state-level field.*"
                )
            else:
                reply_text = (
...
.elvora-small-fallback,.elvora-side-small-fallback{
    width:54px;height:54px;border-radius:50%;display:grid;place-items:center;
    background:linear-gradient(135deg,#0b5d43,#d59a2a);color:#fff;
    font:800 .55rem Inter,sans-serif;letter-spacing:1px;border:2px solid #f2d39b;
    box-shadow:0 5px 16px rgba(0,0,0,.18);
}
.elvora-large-fallback{
    width:460px;max-width:100%;height:260px;margin:auto;border-radius:20px;
    display:grid;place-items:center;background:linear-gradient(135deg,#0b5d43,#d59a2a);
    color:#fff;font:800 4rem Georgia,serif;letter-spacing:3px;
}
.elvora-side-small-fallback{width:68px;height:68px;margin:auto}
...
<div class="home-front">
  <div class="home-nav">
    ...
    <div class="elvora-mark">
      <details>
        <summary><div class="elvora-small-fallback">ELVORA</div></summary>
        <div class="elvora-pop"><div class="elvora-large-fallback">ELVORA</div><div class="elvora-hint">Elvora — Cultural DNA</div></div>
      </details>
    </div>
